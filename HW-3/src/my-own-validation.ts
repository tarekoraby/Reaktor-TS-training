...
export function availability(input: any) {
  string(input);
  const availability = input.match(/(?<=<INSTOCKVALUE>)(.*)(?=<\/INSTOCKVALUE>)/);
  if (!availability) throw Error('Availability-parsing error');

  input = { availability: string };
  input.availability = availability[0].toLowerCase();
  return input as { availability: string };
}
...
type Intersection<A, B> = A extends object ? (B extends object ? ObjectIntersection<A, B> : A & B) : A & B;
...
export function intersection<A, B, C>(
  ...
  b: Validator<B>,
  c: Validator<C>
): Validator<Intersection<A, Intersection<B, C>>>;
