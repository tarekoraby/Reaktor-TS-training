...
const ProdcutDetailsCodec = t.type({
  id: t.string,
  name: t.string,
  type: t.string,
...
type ProdcutDetails = t.TypeOf<typeof ProdcutDetailsCodec>;

type ProdcutDetailsAndAvailability = ProdcutDetails & {
  availability: string;
};

const AvailabilityDetailsCodec = t.type({
  id: t.string,
  DATAPAYLOAD: t.string,
});
type AvailabilityDetails = t.TypeOf<typeof AvailabilityDetailsCodec>;

const AvailabilityDetailsResponseCodec = t.type({
  code: t.number,
  response: t.array(AvailabilityDetailsCodec),
});
type AvailabilityDetailsResponse = t.TypeOf<typeof AvailabilityDetailsResponseCodec>;
...
async function fetchData<T>(url: string, codec: t.Type<T>): Promise<T> {
...
async function updateAvailabilityData(manufacturer: string, availabilityData: AvailabilityData) {
  ...
  availabilityData[manufacturer].forEach((elem) => {
    ...
    if (str !== null) {
      elem.DATAPAYLOAD = str[0].toLowerCase();
    }
  });
