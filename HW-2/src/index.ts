...
async function populateProductData() {
  ...
  const jackets: Promise<Array<ProdcutDetails>> = getProductData('jackets');
  const shirts: Promise<Array<ProdcutDetails>> = getProductData('shirts');
  const accessories: Promise<Array<ProdcutDetails>> = getProductData('accessories');
...
async function getProductData(product: string): Promise<Array<ProdcutDetails>> {
  let payload: Promise<Array<ProdcutDetails>> = fetchData<ProdcutDetails[]>(`${urlRoot}/products/${product}`);
  return payload;
}
...
async function getAvailabilityData(manufacturer: string): Promise<Array<AvailabilityDetails>> {
  let payload = await fetchData<{
    code: string;
    response: Array<AvailabilityDetails>;
  ...
  if (Array.isArray(responseContent)) {
    ...
    console.log(`Retrying to fetch manufacturer data for ${manufacturer}`);
    return getAvailabilityData(manufacturer);
  }
}
